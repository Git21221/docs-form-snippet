...
const Navigation = ({isSearchOpen,setIsSearchOpen}) => {
    ...
    return (
        <div className="h-16 flex items-center p-6 justify-between border-b-2 border-slate-200 sticky top-0 z-10 bg-white">
            ...
            <div id="searchbar" className={`w-64 h-11 rounded-full flex items-center justify-between p-2 cursor-pointer select-none shadow-slate-600 shadow-inner bg-slate-100 ${isSearchOpen ? 'opacity-0': ''}`} onClick={()=>setIsSearchOpen(true)}><FaMagnifyingGlass className="text-slate-600"/><span className="text-slate-600 font-medium">Search</span><span className="shadow-md shadow-slate-400 font-bold bg-slate-100 p-1 text-[12px] rounded-full">Ctrl + Q</span></div>
            <div id="others">
