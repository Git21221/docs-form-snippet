  ...
  return (
    <>
      ...
      <div
        className={`w-[270px] ${
          isScreenSmall
            ? `fixed top-0 left-0 z-20 bg-white h-full transition-transform duration-300 ${
      ...
      >
        <span>
          {sidebarData.map((data) => (
            <SidebarItems key={data.id} data={data} closeSidebar={closeSidebar} />
          ))}
        </span>

        {/* Sidebar Collapse Icon (only visible on small screens) */}
        {isScreenSmall && (
          <button onClick={toggleSidebar} className="text-3xl mb-4 ml-2 cursor-pointer">
      ...
      {isScreenSmall && !isSidebarOpen && (
        <button
          className="fixed bottom-4 left-3 z-20 text-3xl"
